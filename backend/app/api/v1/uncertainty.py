...
async def get_uncertain_segments(
    ...
    for csv_file in results_folder.glob("predictions_*.csv"):
        try:
            ...
            if category:
                filtered = filtered.filter(pl.col(class_col) == category)

            # Add row index for tracking reviewed segments
            filtered_with_idx = filtered.with_row_index("_row_idx")

            for row in filtered_with_idx.iter_rows(named=True):
                idx = row["_row_idx"]
