"""
Test 1D CNN with raw waveform
Direct waveform processing without feature extraction
"""
import sys
...
class CNN1D(nn.Module):
    ...
    Inspired by SampleCNN and M5 architectures
    """
    def __init__(self, num_classes=5):
        super().__init__()

        # First conv layer - learn low-level features from raw waveform
    ...
    def forward(self, x):
        # x shape: (batch, samples) - raw waveform
...
def main():
    print("=" * 80)
    print("TESTING 1D CNN + RAW WAVEFORM")
    print("=" * 80)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            save_path = Path(r"Y:\!_FILHARMONIA\ML_EXPERIMENTS\models\cnn1d_best.pth")
    ...
    checkpoint = torch.load(
        Path(r"Y:\!_FILHARMONIA\ML_EXPERIMENTS\models\cnn1d_best.pth")
    )
    ...
    print(f"\nModel saved to: Y:\\!_FILHARMONIA\\ML_EXPERIMENTS\\models\\cnn1d_best.pth")
    print("=" * 80)
