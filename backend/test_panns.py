"""
Test PANNs (Pre-trained Audio Neural Networks)
Pretrained on AudioSet - should be fast and accurate
"""
import sys
...
class Cnn14(nn.Module):
    ...
    Simplified version for our 5 classes
    """
    ...
    def forward(self, x):
        # x: (batch, freq, time) - mel-spectrogram
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            save_path = Path(r"Y:\!_FILHARMONIA\ML_EXPERIMENTS\models\panns_best.pth")
    ...
    checkpoint = torch.load(
        Path(r"Y:\!_FILHARMONIA\ML_EXPERIMENTS\models\panns_best.pth")
    )
    ...
    # Summary
    print("\n" + "=" * 80)
    print("PANNs CNN14 - RESULTS")
    print("=" * 80)
    print(f"\nBest Validation Accuracy: {best_val_acc:.2f}%")
    print(f"Final Test Accuracy: {test_acc:.2f}%")
    print(f"Training Time: {training_time / 60:.2f} minutes")
    print(f"Epochs Trained: {epoch + 1}")
    print(f"\nModel saved to: Y:\\!_FILHARMONIA\\ML_EXPERIMENTS\\models\\panns_best.pth")
    print("=" * 80)
