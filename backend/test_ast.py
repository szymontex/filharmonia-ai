"""
Test Audio Spectrogram Transformer (AST) on our dataset
Uses pretrained AST model from HuggingFace
"""
import sys
...
def main():
    print("=" * 80)
    print("TESTING AUDIO SPECTROGRAM TRANSFORMER (AST)")
    print("=" * 80)

    # Hyperparameters
    BATCH_SIZE = 8  # Small batch due to limited data
    LEARNING_RATE = 1e-4
    EPOCHS = 10
    NUM_CLASSES = 5
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            save_path = Path(r"Y:\!_FILHARMONIA\ML_EXPERIMENTS\models\ast_best.pth")
    ...
    checkpoint = torch.load(
        Path(r"Y:\!_FILHARMONIA\ML_EXPERIMENTS\models\ast_best.pth")
    )
    ...
    # Summary
    print("\n" + "=" * 80)
    print("AUDIO SPECTROGRAM TRANSFORMER (AST) - RESULTS")
    print("=" * 80)
    print(f"\nBest Validation Accuracy: {best_val_acc:.2f}%")
    print(f"Final Test Accuracy: {test_acc:.2f}%")
    print(f"Training Time: {training_time / 60:.2f} minutes")
    print(f"Epochs Trained: {EPOCHS}")
    print(f"\nModel saved to: Y:\\!_FILHARMONIA\\ML_EXPERIMENTS\\models\\ast_best.pth")
    print("=" * 80)
